...
def password_home(request):
    allUserCategories = PasswordCategory.objects.filter(
        created_by_id=request.user.id)
    allUserPasswords = PasswordHint.objects.filter(
        created_by_id=request.user.id)
...
def create_category(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            categoryObj = PasswordCategory(
                category_name=formData['category_name'], created_by=request.user)
            categoryObj.save()
...
def create_password(request):
    if request.method == 'POST':
        ...
        does_category_exist = PasswordCategory.objects.filter(
            created_by=request.user)
    ...
    if request.method == 'POST':
        try:
            ...
            if request.FILES.get('hint_image_changed'):
                password_hint_obj.hint_image = request.FILES.get(
                    'hint_image_changed')
...
def update_generated_password(request, pk):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            generated_password_obj.encrypted_password = encoded_text.decode(
                'utf-8')
...
def list_generated_password(request):
    generated_user_passwords = GeneratedPassword.objects.filter(
        created_by_id=request.user.id)
...
def upload_file(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Encryption using secret key and cryptography module
            secret_key = (request.user.user_secret_key)
            cipher_suite = Fernet(secret_key)
            ...
            # file_object.save()
            try:
                content = ContentFile(base64.b64encode(encrypted_data))
                user_file_name = str(request.user.username) + \
                    get_random_string(3) + '.txt'
...
def file_detail_view(request, pk):
    ...
    if not bool(file_obj.decrypted_file.name):
        encrypted_file_data = file_obj.actual_file.file.read()
        dc = base64.b64decode(encrypted_file_data)
        secret_key = (request.user.user_secret_key).encode()
        fer = Fernet(secret_key)
        data = fer.decrypt(dc)
        data=data.decode('utf-8').replace('\n','')
        content = ContentFile(data.encode())
        user_file_name = str(request.user.username) + get_random_string(3) + '.txt'
        file_obj.decrypted_file.save(user_file_name, content)
        file_obj.save()
    ...
    return render(request, 'passwords/file_detail.html', {'file_obj': file_obj, 'file_path': file_obj.decrypted_file.url})
...
#         path='https://grp-19.s3.amazonaws.com/file_uploads/s3GvH.txt'
#         print(path)
#         with open(path,'rb') as fh:
#             response=HttpResponse(fh.read(),content_type="application/decrypted_file")
