...
class PasswordHint(models.Model):
    linked_category = models.ForeignKey(
        PasswordCategory, on_delete=models.CASCADE)
...
class GeneratedPassword(models.Model):
    ...
    security_level = models.CharField(
        max_length=50, choices=PASSWORD_SECURITY_CHOICES)
...
class FileEncrypt(models.Model):
    ...
    file_description = models.CharField(
        "Description of the file", max_length=200)
    actual_file = models.FileField(upload_to='file_uploads')
    decrypted_file = models.FileField(upload_to='file_uploads', default=None)
    ...
    def get_decr_file_size(self):
        size_bytes = self.decrypted_file.size
        if size_bytes == 0:
            return "0B"
        size_name = ("B", "KB", "MB", "GB", "TB", "PB", "EB", "ZB", "YB")
        i = int(math.floor(math.log(size_bytes, 1024)))
